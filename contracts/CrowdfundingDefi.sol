...
contract CrowdfundingDefi is Ownable {
    ...
    // Series A
    function openSeriesAFunding(uint256 _fundingRoundDeadline)
        external
        onlyOwner
        startFunding
    {
        // setting the funding deadline
        fundingRoundDeadline =
            block.timestamp +
            (_fundingRoundDeadline * 1 days);
        // updating the funding state
    ...
    // Series B
    function openSeriesBFunding(uint256 _fundingRoundDeadline)
        external
        onlyOwner
        startFunding
    {
    ...
    // Series C
    function openSeriesCFunding(uint256 _fundingRoundDeadline)
        external
        onlyOwner
        startFunding
    {
    ...
    function closeFundingRound() external onlyOwner returns (uint256) {
        ...
        require(
            ...
            "Time still remains in this funding round."
        );
    ...
    function fund(uint256 _amount) external {
        require(
            fundingRoundDeadline >= block.timestamp,
            "No funding round is currently open"
        );
        require(
        ...
        // transferring the WETH
        WETH.transferFrom(msg.sender, address(this), _amount);
        // updating the funding raised
        fundingRaised += _amount;
        // updating the array of people who funded
    ...
    function _yieldFarm() internal {
        ...
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
            .ExactInputSingleParams({
                ...
                amountIn: leftOver,
                amountOutMinimum: 0, // change this later
        ...
        // calculating how much usdt we have
    ...
    // when time has hit threshold, can withdraw from lending pool
    function endYieldFarming() external onlyOwner {
        require(
            block.timestamp >= endOfYieldPeriod,
            "Yielding cannot end yet."
        );

        // calculate total balance on aave
        uint256 totalBalance = aUSDT.balanceOf(address(this));
